...
class MainWindowBackend(GUIBACK_BASE):
    ...
    def select_gid(back, gid, imgsetid=None, show=True, sel_aids=None,
        ...
        if sel_aids is None:
            ...
            if len(sel_aids) > 0:
                ...
                sel_aids = []
        logger.info('[back] select_gid(gid=%r, imgsetid=%r, sel_aids=%r)' % (
    ...
    def train_rf_with_imageset(back, **kwargs):
        ...
        if species == 'none':
            species = None
        logger.info("[train_rf_with_imageset] Training Random Forest trees with imgsetid=%r and species=%r" % (imgsetid, species, ))
    ...
    def merge_imagesets(back, imgsetid_list, destination_imgsetid):
        ...
        logger.info('[back] merge_imagesets: %r, %r' % (destination_imgsetid, imgsetid_list))
        if back.contains_special_imagesets(imgsetid_list):
            back.display_special_imagesets_error()
            return
        ibs = back.ibs
        try:
            ...
            logger.info('[back] merge_imagesets cannot find index for %r' % (destination_imgsetid,))
            destination_index = 0
    ...
    def run_detection_step(back, refresh=True, **kwargs):
        ...
        if True:
            ...
            if detector in ['cnn', 'cnn_yolo', 'yolo']:
                ...
                msg_str = msg_fmtstr.format(**fmtdict)
                if back.are_you_sure(use_msg=msg_str):
                    logger.info('[back] run_detection_step(species=%r, imgsetid=%r)' % (species, imgsetid))
                    ibs.detect_random_forest(gid_list, species)
                    logger.info('[back] about to finish detection')
                    if refresh:
                        back.front.update_tables([gh.IMAGE_TABLE, gh.ANNOTATION_TABLE])
    ...
    def compute_queries(back, refresh=True, daids_mode=None,
        ...
        logger.info('\n')
        logger.info('------')
        logger.info('[back] compute_queries: imgsetid=%r, mode=%r' % (imgsetid, back.daids_mode))
        logger.info('[back] use_prioritized_name_subset = %r' % (use_prioritized_name_subset,))
    ...
    def _get_expanded_aids_groups(back, imgsetid, daids_mode=None,
        ...
        if use_prioritized_name_subset:
            ...
            logger.info('[back] Filtered query by quality and viewpoint: len(qaid_list) = %r' % (len(qaid_list),))
        ...
        for species in species_list:
            ...
            if len(qaids_) > 0 and len(daids) > 0:
                ...
                logger.info('[back] ! len(nospecies_qaids) = %r' % (len(nospecies_qaids),))
    ...
    def export_database_as_kwcoco(back):
        """ File -> Export Database """
        logger.info('[back] export_database_as_kwcoco')
        ...
        if gid_list is not None and len(gid_list) > 0:
            if smart_xml_fpath is None:
                ...
                try:
                    ...
                    back.ibs.delete_images(gid_list)
                    logger.info(('[back] ERROR: Parsing Patrol XML file failed, '
    ...
    def import_images_from_encounters(back, level=1, dir_list=None, size_filter=None, refresh=True,
        ...
        if len(warning_set) > 0:
            ...
            args = (warning_list, )
            logger.info('[guiback] WARNING: Some files in the encounters will not be imported: %r' % args)
