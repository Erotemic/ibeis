...
def sync_wildbook():
    ...
    logger.info('The IA database has %r/%r images not from this downloaded set' % (
...
def sync_existing_images(ibs, hit_info, species, DRY):
    ...
    if not any(dirty_flags):
        ...
        logger.info('...There are %d/%d original uris that need fixing' % (sum(dirty_flags), len(dirty_flags)))
...
def sync_annot_info(ibs, single_annots, single_info, species, DRY):
    ...
    logger.info('unioning new injur tags into %r/%r annots' % (sum(isdirty), len(isdirty)))
    if not DRY:
    ...
    dirty_info = single_info.compress(isdirty)
    logger.info('new_keywords' + ut.repr4(ut.dict_hist(ut.flatten(single_info['new_keywords']))))
    logger.info('unioning new_keywords into %r/%r annots' % (len(dirty_info), len(isdirty)))
    if not DRY:
    ...
    logger.info('Tags from WB imgnames:' +
...
def check_annot_disagree(single_info, single_annots, key1, prop2, repl2,
    ...
    logger.info('Prop=%r has %r/%r out of sync items' % (key1, sum(out_of_sync),
                                                   len(out_of_sync)))
    logger.info('WB has populated info for %r/%r %r' % (sum(ia_empty),
                                                  len(ia_empty), key1))
    logger.info('IA has populated info for %r/%r %r' % (sum(wb_empty),
                                                  len(wb_empty), key1))
    logger.info('IA and WB disagree on info for %r/%r %r' % (sum(disagree),
                                                       len(disagree), key1))
    if is_set:
        logger.info('IA is subset of WB info for %r/%r %r' % (
            sum(ia_is_subset), len(ia_is_subset), key1))
        logger.info('WB is subset of IA info for %r/%r %r' % (
            sum(wb_is_subset), len(wb_is_subset), key1))
        logger.info('WB and IA partial overlap info for %r/%r %r' % (
            sum(some_isect), len(some_isect), key1))
        logger.info('IA and WB total disagree on info for %r/%r %r' % (
    ...
    if is_set:
        logger.info(r'IA \subset WB (can give)')
        sub_info.compress(ia_is_subset).print()
        logger.info(r'WB \subset IA (updates on IA side?)')
...
def get_injur_categories(single_annots, verbose=False):
    ...
    def fixinjur(aid, tags):
        ...
        if injured:
            if 'healthy' in tags:
                logger.info('shark aid=%r labeled as injured and healty %r!!!' % (aid, tags,))
        if len(tags) == 0:
            return tags
    ...
    if verbose:
        ...
        logger.info('mapping: ' + ut.repr3(ut.group_items(alias_map.keys(),
                                                    alias_map.values())))
        ...
        logger.info('used_mapping: ' + ut.repr3(ut.group_items(alias_map_used.keys(),
...
def add_new_images(ibs, miss_info, species):
    ...
    if verbose:
        ...
        logger.info('Of these, %r images had other tags' % (sum(ut.lmap(bool, other_keywords))))
        logger.info('Of these, %r images had no injured tags' % (
    ...
    if ibs.dbname == 'WS_ALL':
        ...
        logger.info('%d/%d are missing localizations' % (len(failed_gids), len(results_list)))
        logger.info('%d/%d had multiple localizations' % (len(multi_gids), len(results_list)))
    ...
    logger.info(f'aid_list={aid_list}')
...
def parse_whaleshark_org_keywords():
    ...
    if verbose:
        ...
        logger.info('Of these, %r images had injured tags' % (sum(ut.lmap(bool, injured_keywords))))
        logger.info('Of these, %r images had other tags' % (sum(ut.lmap(bool, other_keywords))))
        logger.info('Of these, %r images had no injured tags' % (
