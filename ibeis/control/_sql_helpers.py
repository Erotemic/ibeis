...
def database_backup(db_dir, db_fname, backup_dir, max_keep=MAX_KEEP, manual=True):
    ...
    if exists(src_fpath) and not exists(dst_fpath):
        logger.info('[ensure_daily_database_backup] Daily backup of database: %r -> %r' % (src_fpath, dst_fpath, ))
...
def remove_old_backups(backup_dir, ext, max_keep):
    ...
    if len(path_list) > max_keep:
        ...
        for path_delete in path_delete_list:
            logger.info('[ensure_daily_database_backup] Deleting old backup %r' % path_delete)
    ...
    if can_skip:
        ...
        if current_schema_exists:
            ...
            if current_schema_compatible:
                ...
                if verbose:
                    logger.info('[_SQL] Database version updated (skipped) to %r ' % (schema_spec.VERSION_CURRENT))
            else:
                logger.info('[_SQL] Current database is not compatible, updating incrementally...')
        else:
            logger.info('[_SQL] New database but current version not exported, updating incrementally...')
    #+--------------------------------------
...
def autogenerate_nth_schema_version(schema_spec, n=-1):
    ...
    logger.info(' Run with --diff or --diff=<numcontextlines> to see the difference between current and requested')
    return db
