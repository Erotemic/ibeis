...
def ishow_image(ibs, gid, sel_aids=[], fnum=None, select_callback=None,
                **kwargs):
    if ut.VERBOSE:
        logger.info(ut.get_caller_name(range(9)))
        logger.info('[interact_image] gid=%r fnum=%r' % (gid, fnum,))
    if fnum is None:
        fnum = df2.next_fnum()
    ...
    def _on_image_click(event):
        logger.debug('[inter] clicked image')
        if ih.clicked_outside_axis(event):
            ...
            logger.debug(' annotation_centers=%r' % annotation_centers)
            logger.debug(' viztype=%r' % viztype)
            if len(annotation_centers) == 0:
                logger.info(' ...no chips exist to click')
                return
